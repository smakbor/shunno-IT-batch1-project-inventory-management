...
} from '../../redux/services/mediaService';
            ...
            <Col xs={6} md={2}>
                <Card style={{ width: 'auto', height: 'auto' }} className="m-0 mb-2">
                    ...
                    <Card.Body className="px-1 py-0">
                        <h6 style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                            {item.name}
                        </h6>
                        ...
                        <div className="d-flex justify-content-between">
                            <div>
                                <i
                                    className="dripicons-expand text-dark btn"
                                    onClick={() => toggleEnlarged(item._id)}
                                />
                            </div>
                            <div>
                                <i
                                    className="dripicons-trash text-danger btn"
                                    onClick={() =>
                                        AleartMessage.DeleteFile({ id: item._id, store: store._id }, mediaDelete)
                                    }
                                />
                            </div>
                        </div>
            ...
            <>
                ...
                <div className="d-flex justify-content-start align-items-center">
                    ...
                    {selected.length > 0 && (
                        <Button
                            variant="danger"
                            className="mb-2 d-block ms-3"
                            disabled={multiDeleteLoading}
                        ...
                        <Card>
                            <Card.Body>
                                <Row className="mb-2 mt-0">
                                    {data.length === 0 && (
                                        <>
                                            <p className="text-muted fs-1 text-center">No File Uploaded!</p>
                                        </>
                                    )}
                                    {data && data.map((item) => <FileCardComponent item={item} key={item._id} />)}
                                </Row>
